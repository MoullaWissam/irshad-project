...
export function BotAvatar(props) {
  const group = useRef();
  const { scene, animations } = useGLTF('/models/Murshed-compressed.glb');
  const { actions } = useAnimations(animations, group);
  ...
  useFrame((state) => {
    if (group.current) group.current.rotation.y = Math.sin(state.clock.elapsedTime * 0.5) * 0.1;
  });

  if (!scene) return <LoadingIndicator progress={0} />;
